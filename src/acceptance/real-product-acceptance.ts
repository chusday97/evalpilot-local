...
const scenarioPreflightSchema = z.object({
  ...
  prerequisitePlans: z.array(z.object({
    caseId: z.string(), status: z.string(), reasons: z.array(z.string()).optional(),
    unresolvedBlockers: z.array(z.object({ type: z.string(), summary: z.string().optional(), sourceValue: z.string().optional() }).passthrough()).optional(),
  }).passthrough()).default([]),
  scenarios: z.array(z.object({ caseId: z.string(), goal: z.string(), startingUrl: z.string().optional() }).passthrough()).default([]),
}).passthrough();

const adaptiveReportSubsetSchema = z.object({
  caseResults: z.array(z.object({
    caseId: z.string(), verdict: z.enum(['pass', 'fail', 'inconclusive']), failureSource: z.string().nullable().optional(),
    semantic: z.object({ summary: z.string().optional() }).passthrough().optional(),
  }).passthrough()).default([]),
}).passthrough();

const foundationQualitySubsetSchema = z.object({ quality: z.enum(['ready', 'degraded', 'invalid']), warnings: z.array(z.string()).optional() }).passthrough();

export type AcceptanceTaskStatus = 'pass' | 'product_failure' | 'evaluator_failure' | 'prerequisite_blocked' | 'not_run' | 'not_generated' | 'ambiguous_scenario' | 'foundation_blocked';

export interface RealProductAcceptanceTaskResult { acceptanceTaskId: string; name: string; status: AcceptanceTaskStatus; caseId: string | null; reason: string; }
export interface RealProductAcceptanceGateResult {
  product: string; repository: string; commit: string; passed: boolean; taskCompletionRate: number;
  counts: { planned: number; passed: number; productFailures: number; evaluatorFailures: number; prerequisiteBlocks: number; notRun: number; notGenerated: number; ambiguous: number; foundationBlocked: number; };
  tasks: RealProductAcceptanceTaskResult[]; failedThresholds: string[];
}
...
export function evaluateRealProductAcceptance(input: { manifest: RealProductAcceptanceManifest; preflight: unknown; report: unknown; foundationQuality?: unknown }): RealProductAcceptanceGateResult {
  ...
  const tasks: RealProductAcceptanceTaskResult[] = manifest.tasks.map((task) => {
    if (foundation && foundation.quality !== 'ready') return { acceptanceTaskId: task.acceptanceTaskId, name: task.name, status: 'foundation_blocked', caseId: null, reason: `Foundation quality=${foundation.quality}${foundation.warnings?.length ? `：${foundation.warnings.join('；')}` : ''}` };
    const matching = preflight.scenarios.filter((scenario) => matchesGoal(scenario.goal, task.goalIncludes));
    if (!matching.length) return { acceptanceTaskId: task.acceptanceTaskId, name: task.name, status: 'not_generated', caseId: null, reason: `自动生成的 Scenario 中没有找到同时包含“${task.goalIncludes.join(' / ')}”的真实任务。` };
    if (matching.length > 1) return { acceptanceTaskId: task.acceptanceTaskId, name: task.name, status: 'ambiguous_scenario', caseId: null, reason: `有 ${matching.length} 个 Scenario 同时匹配该验收任务，EvalPilot 不能安全猜测应该使用哪一个。` };
    const scenario = matching[0]!;
    if (blockedCaseIds.has(scenario.caseId)) return { acceptanceTaskId: task.acceptanceTaskId, name: task.name, status: 'prerequisite_blocked', caseId: scenario.caseId, reason: blockerReason(preflight, scenario.caseId) };
    const result = resultByCaseId.get(scenario.caseId);
    if (!result) return { acceptanceTaskId: task.acceptanceTaskId, name: task.name, status: 'not_run', caseId: scenario.caseId, reason: 'Case 已生成且未被 Preflight 阻止，但本次 evaluation 没有该 Case 的 Judge 结果。' };
    if (result.verdict === 'pass') return { acceptanceTaskId: task.acceptanceTaskId, name: task.name, status: 'pass', caseId: scenario.caseId, reason: result.semantic?.summary || 'Judge 已确认任务通过。' };
    if (result.verdict === 'fail' && result.failureSource === 'product') return { acceptanceTaskId: task.acceptanceTaskId, name: task.name, status: 'product_failure', caseId: scenario.caseId, reason: result.semantic?.summary || 'Judge 已确认 Product Failure。' };
    return { acceptanceTaskId: task.acceptanceTaskId, name: task.name, status: 'evaluator_failure', caseId: scenario.caseId, reason: result.semantic?.summary || `Judge verdict=${result.verdict}，failureSource=${result.failureSource ?? 'unknown'}。` };
  });

  const count = (status: AcceptanceTaskStatus) => tasks.filter((task) => task.status === status).length;
  const counts = { planned: tasks.length, passed: count('pass'), productFailures: count('product_failure'), evaluatorFailures: count('evaluator_failure'), prerequisiteBlocks: count('prerequisite_blocked'), notRun: count('not_run'), notGenerated: count('not_generated'), ambiguous: count('ambiguous_scenario'), foundationBlocked: count('foundation_blocked') };
  ...
  return { product: manifest.product.name, repository: manifest.product.repository, commit: manifest.product.commit, passed: failedThresholds.length === 0, taskCompletionRate: counts.planned ? counts.passed / counts.planned : 0, counts, tasks, failedThresholds };
}

export async function evaluateRealProductAcceptanceFromArtifacts(input: { manifestPath: string; evaluationDirectory: string }): Promise<RealProductAcceptanceGateResult> {
  ...
  return evaluateRealProductAcceptance({ manifest, preflight: JSON.parse(preflightText), report: JSON.parse(reportText), foundationQuality: JSON.parse(foundationText) });
}
