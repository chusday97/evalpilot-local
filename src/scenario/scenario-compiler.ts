...
export interface ScenarioExecutionPlan {
  ready: ExecutableScenario[];
  blocked: ExecutableScenario[];
  readyCaseIds: string[];
...
const testDataPatterns = [/\b(?:test file|fixture file|uploaded file|sample file|seed data|test data)\b/i, /(?:测试文件|样例文件|待上传文件|测试数据|样例数据|种子数据)/];
...
const normalized = (value: string): string => value.toLocaleLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]/g, '');
...
function specificPreconditionBlocker(caseId: string, index: number, text: string): ScenarioBlocker | null {
  if (matchesAny(text, authPatterns)) return { blockerId: `${caseId}-precondition-${index + 1}`, type: 'needs_auth', summary: '这个任务需要可复用的测试登录态，当前还没有安全的认证 Fixture。', source: 'precondition', sourceValue: text };
  if (matchesAny(text, testDataPatterns)) return { blockerId: `${caseId}-precondition-${index + 1}`, type: 'needs_test_data', summary: '这个任务依赖测试文件或测试数据，当前 Scenario 还没有对应 Fixture。', source: 'precondition', sourceValue: text };
  if (matchesAny(text, humanInputPatterns)) return { blockerId: `${caseId}-precondition-${index + 1}`, type: 'needs_human_input', summary: '这个任务依赖尚未确认的业务条件，不能由评测器自行猜测。', source: 'precondition', sourceValue: text };
  if (matchesAny(text, setupPatterns)) return { blockerId: `${caseId}-precondition-${index + 1}`, type: 'needs_setup', summary: '这个任务依赖已有对象或历史状态，需要先执行 Setup。', source: 'precondition', sourceValue: text };
  return null;
}
...
function startingUrlFor(evalCase: EvalCase, model: ProductModel, targetUrl: string): { url: string; blocker: ScenarioBlocker | null } {
  ...
  try { return { url: new URL(entry, targetUrl).toString(), blocker: null }; }
...
export function compileExecutableScenario(input: { evalCase: EvalCase; productModel: ProductModel; targetUrl: string; generatedAt?: string }): ExecutableScenario {
  ...
  if (input.evalCase.taskId && !resolvedTask) blockers.push({ blockerId: `${input.evalCase.caseId}-missing-task`, type: 'unsupported', summary: '这个案例引用的 Product Task 已不存在，不能安全执行。', source: 'product_model', sourceValue: input.evalCase.taskId });
  const start = startingUrlFor(input.evalCase, input.productModel, input.targetUrl);
  if (start.blocker) blockers.push(start.blocker);
  const sourcePreconditions = [...new Set([...input.evalCase.preconditions, ...(resolvedTask?.preconditions ?? [])].map((item) => item.trim()).filter(Boolean))];
...
export function compileExecutableScenarios(input: { cases: EvalCase[]; productModel: ProductModel; targetUrl: string; generatedAt?: string }): ExecutableScenario[] {
  return input.cases.map((evalCase) => compileExecutableScenario({ evalCase, productModel: input.productModel, targetUrl: input.targetUrl, generatedAt: input.generatedAt }));
}

export function planScenarioExecution(scenarios: ExecutableScenario[]): ScenarioExecutionPlan {
  const ready = scenarios.filter((scenario) => scenario.readiness === 'ready');
  const blocked = scenarios.filter((scenario) => scenario.readiness !== 'ready');
  return { ready, blocked, readyCaseIds: ready.map((scenario) => scenario.caseId), blockedCaseIds: blocked.map((scenario) => scenario.caseId), canRunAny: ready.length > 0, allBlocked: scenarios.length > 0 && ready.length === 0 };
}

export function scenarioBlockerSummary(scenarios: ExecutableScenario[]): string {
  const blocked = scenarios.filter((scenario) => scenario.readiness !== 'ready');
  if (!blocked.length) return '';
  return blocked.map((scenario) => `${scenario.caseId}: ${scenario.blockers.map((blocker) => blocker.sourceValue ? `${blocker.summary}（${blocker.sourceValue}）` : blocker.summary).join('；')}`).join(' | ');
}
