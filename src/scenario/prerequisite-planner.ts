...
import { resolveScenarioSetup, type AutoSetupPlan } from './setup-resolver.js';
...
export async function planScenarioPrerequisites(input: {
  ...
  if (setupBlockers.length) {
    ...
    const setup = resolveScenarioSetup({
    ...
    if (setup.status === 'auto_setup' && setup.plan) setupPlan = setup.plan;
