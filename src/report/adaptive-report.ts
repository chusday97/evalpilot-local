...
import { z } from 'zod';
import type { AdaptiveEvaluationReport, Badcase, CandidateFinding, CoverageMatrix, EvalCase, EvalCaseResult, EvidencePacket, FixTask, WorkflowStatus } from '../../types.js';
import { decideEvaluationNextAction } from '../decision/next-action-engine.js';
import { evaluationNextActionSchema } from '../decision/schemas.js';
...
export async function buildAdaptiveEvaluationReport(input: { outputDir: string; projectId: string; evaluationId?: string; evaluationStatus?: WorkflowStatus; selectedCases: EvalCase[]; results: EvalCaseResult[]; packets: EvidencePacket[]; coverage: CoverageMatrix | null; findings?: CandidateFinding[]; badcases?: Badcase[]; fixTasks?: FixTask[]; regressionCases?: EvalCase[]; challengeCases?: EvalCase[]; generatedAt?: string }): Promise<AdaptiveEvaluationReport> {
  ...
  const report = adaptiveEvaluationReportSchema.parse({
    ...
    newBadcaseIds: (input.badcases ?? []).map((item) => item.badcaseId), newRegressionCaseIds: (input.regressionCases ?? []).map((item) => item.caseId), passingCoverageGaps: input.coverage?.gaps ?? [], newChallengeCaseIds: (input.challengeCases ?? []).map((item) => item.caseId), nextAction, recommendedNextActions,
