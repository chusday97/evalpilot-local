import type { EvalCase, EvalCaseResult, EvidencePacket, PassAnalysis, ProductModel } from '../../types.js';
...
export function analyzePassingCase(input: { evalCase: EvalCase; result: EvalCaseResult; evidencePacket?: EvidencePacket; model: ProductModel; existingCases: EvalCase[]; generatedAt?: string }): PassAnalysis {
  ...
  const matrix = analyzeCoverage({ model: input.model, cases: input.existingCases, results: [input.result], evidencePackets: input.evidencePacket ? [input.evidencePacket] : [], generatedAt: input.generatedAt });
