...
export type EvaluationRuntime = 'legacy' | 'adaptive';
...
export type AiProviderName = 'openai' | 'deepseek' | 'kimi' | 'openai_compatible';
export type AiProviderProtocol = 'responses' | 'chat_completions';
export interface AiProviderConnectionStatus { provider: AiProviderName | null; displayName: string | null; configured: boolean; source: AiProviderConnectionSource; protocol: AiProviderProtocol | null; apiHost: string | null; model: string | null; connectedAt: string | null }
export type AiProviderConnectionRequest =
  | { provider: Exclude<AiProviderName, 'openai_compatible'>; apiKey: string; model?: string; confirmed: true }
  | { provider: 'openai_compatible'; apiKey: string; baseUrl: string; model: string; confirmed: true };
...
/** 实时探测快照；持久化的 ProjectProfile.status 不能替代 canEvaluate。 */
...
export interface EvaluationSession { evaluationId: string; projectId: string; sequenceNumber: number; runtime: EvaluationRuntime; depth: EvaluationDepth; capabilityIds: string[]; capabilityNames: string[]; plannedCapabilityIds: string[]; plannedCapabilityNames: string[]; executedCapabilityIds: string[]; executedCapabilityNames: string[]; selectedCaseIds: string[]; coverage: EvaluationCoverageSummary | null; coverageMatrix: CoverageMatrix | null; customName: string | null; competitorSnapshotIds: string[]; issueIds: string[]; findingIds: string[]; badcaseIds: string[]; remoteModelAuthorized: boolean; screenshotAuthorized: boolean; status: WorkflowStatus; currentStage: EvaluationStageName; stages: EvaluationStageState[]; runIds: string[]; startedAt: string; completedAt: string | null; error: string | null }
export interface EvaluationOrchestratorInput { projectId: string; evaluationId: string; depth: EvaluationDepth; capabilityIds: string[]; allowRemoteModel: true; allowScreenshot: boolean; /** @deprecated legacy evaluation runtime compatibility marker; normal evaluation always uses false. */ legacyFallback?: false }
export interface EvaluationOrchestratorResult { evaluationId: string; selectedCaseIds: string[]; runIds: string[]; results: EvalCaseResult[]; findings: CandidateFinding[]; badcases: Badcase[]; coverage: CoverageMatrix }
export interface EvaluationFoundationState { schemaVersion: 1; sourceFingerprint: string; productModelVersion: number; evalSetVersion: number; generatedAt: string }
