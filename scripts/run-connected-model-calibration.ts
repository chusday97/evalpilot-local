...
async function main(): Promise<void> {
  ...
  if (jsonOutput) {
    ...
    process.stdout.write([
      ...
      `Provider failure rate: ${variance.providerFailureRate.toFixed(3)} (${variance.providerFailureCount}/${variance.probeExecutionCount})`,
      `Evaluator failure rate: ${variance.evaluatorFailureRate.toFixed(3)} (${variance.evaluatorFailureCount}/${variance.probeExecutionCount})`,
      ...variance.probeStability.map((probe) => `${probe.probeId}: eligible=${probe.eligibleRunCount}/${probe.runCount} signal-exact=${probe.exactSignalMatchRate.toFixed(3)} verdict-match=${probe.verdictMatchRate.toFixed(3)} provider-failure=${probe.providerFailureRate.toFixed(3)} evaluator-failure=${probe.evaluatorFailureRate.toFixed(3)}`),
