...
function baseCase(input: {
  ...
  return {
    ...
    preconditions: [],
    oracle: {
      // The secret marker exists only to prove the real Judge contract is not serialized to
      // Actor requests. The Actor may still legitimately observe actual success text in the UI.
      expectedOutcome: [...input.expectedOutcome, oracleOnlyMarker],
      mustObserve: [],
      mustNotObserve: [],
      businessRules: [],
...
const cases = [
  baseCase({
    ...
    expectedOutcome: ['鱼缸主页面显示已保存尺寸和淡水水体'],
    assertions: [
  ...
  baseCase({
    ...
    expectedOutcome: ['当前鱼缸保存一只 Corydoras aeneus'],
    assertions: [
  ...
  baseCase({
    ...
    expectedOutcome: ['页面显示高风险结果和立即行动建议'],
    assertions: [
...
function findButton(input: any, matches: (value: string) => boolean) {
  return (input.observation?.interactableElements ?? []).find((item: any) =>
    !item.disabled && item.tagName === 'button' && matches(label(item)));
}

function click(input: any, matches: (value: string) => boolean, intentSummary: string, expectedResult: string) {
  const target = findButton(input, matches);
  return target
    ? { intentSummary, action: 'click', targetElementId: target.elementId, value: null, expectedResult, confidence: 0.9 }
    : null;
}
...
function actorDecision(input: any) {
  ...
  if (/Loading AquaGuide/i.test(visible)) {
    return { intentSummary: '页面还在初始化，先等待可见内容出现', action: 'wait', targetElementId: null, value: null, expectedResult: '页面出现可操作内容', confidence: 0.95 };
  }
  ...
  if (goal.includes('60×30×30') && goal.includes('淡水')) {
    if (/60x30x30cm/i.test(visible) && /淡水|Freshwater/i.test(visible) && !/Save Settings/.test(visible)) {
      return { intentSummary: '目标尺寸和淡水状态已经在鱼缸主页面可见，任务完成', action: 'finish', targetElementId: null, value: null, expectedResult: '当前可见状态与自己的建缸目标一致', confidence: 0.95 };
    }
    if (pageUrl.includes('/welcome')) {
      return click(input, (value) => /Build your first aquarium|建立第一个鱼缸/.test(value), '选择与“建立鱼缸”最直接相关的入口', '进入鱼缸创建流程')
        ?? { intentSummary: '欢迎页找不到与建缸目标匹配的入口', action: 'abandon', targetElementId: null, value: null, expectedResult: '停止并保留当前页面证据', confidence: 0.7 };
    }
    const buttons = observation.interactableElements ?? [];
    const saveVisible = buttons.some((item: any) => /Save Settings|保存设置/.test(label(item)));
    if (!saveVisible) {
      return click(input, (value) => /Create or configure a tank|Build or complete aquarium|建立或完善鱼缸|Tank Settings|鱼缸设置/.test(value), '从鱼缸页选择设置/建缸入口', '出现尺寸和水体设置')
        ?? { intentSummary: '鱼缸页找不到可建立或设置鱼缸的安全入口', action: 'abandon', targetElementId: null, value: null, expectedResult: '停止并保留证据', confidence: 0.65 };
    }
    ...
    if (empty) {
      ...
      return { intentSummary: `填写自己已知的第 ${index + 1} 个尺寸`, action: 'fill', targetElementId: empty.elementId, value, expectedResult: '该尺寸字段出现输入值', confidence: 0.95 };
    }
    const dimensions = findButton(input, (value) => /Dimensions|尺寸/.test(value));
    const parameters = findButton(input, (value) => /Parameters|参数/.test(value));
    const freshwater = findButton(input, (value) => /淡水|Freshwater/.test(value) && !/海水|Saltwater/.test(value));
    if (freshwater && /Water type unknown|水体未记录/.test(label(parameters ?? {}))) {
      return { intentSummary: '选择自己已知的淡水类型', action: 'click', targetElementId: freshwater.elementId, value: null, expectedResult: '水体类型显示为淡水', confidence: 0.95 };
    }
    if (dimensions && /Incomplete dimensions|尺寸未记录/.test(label(dimensions))) {
      return { intentSummary: '尺寸还没记录，打开尺寸设置', action: 'click', targetElementId: dimensions.elementId, value: null, expectedResult: '显示长宽高输入框', confidence: 0.9 };
    }
    if (parameters && /Water type unknown|水体未记录/.test(label(parameters))) {
      return { intentSummary: '水体类型还没记录，打开参数设置', action: 'click', targetElementId: parameters.elementId, value: null, expectedResult: '出现淡水/海水选项', confidence: 0.9 };
    }
    return click(input, (value) => /Save Settings|保存设置/.test(value), '当前已填入自己的尺寸和水体信息，保存设置', '返回鱼缸主页面并保留当前配置')
      ?? { intentSummary: '设置已填写但没有看到保存入口', action: 'abandon', targetElementId: null, value: null, expectedResult: '停止并保留证据', confidence: 0.6 };
  }

  if (goal.includes('Corydoras aeneus')) {
    if (/Recorded/.test(visible) && /Corydoras aeneus x 1/.test(visible)) {
      return { intentSummary: '页面明确显示 Recorded 和目标物种数量，记录目标已完成', action: 'finish', targetElementId: null, value: null, expectedResult: '当前可见记录与自己的目标一致', confidence: 0.98 };
    }
    const search = (observation.formFields ?? []).find((field: any) => /Search fish, shrimp, snails|搜索鱼、虾、螺/.test(String(field.placeholder ?? field.label ?? '')));
    if (search && !search.currentValuePresent) {
      return { intentSummary: '搜索自己要记录的学名', action: 'fill', targetElementId: search.elementId, value: 'Corydoras aeneus', expectedResult: '候选列表出现目标物种', confidence: 0.95 };
    }
    const save = click(input, (value) => value === '保存到鱼缸' || /Save to.*tank/i.test(value), '把当前选择保存到鱼缸', '页面确认记录已保存');
    if (save) return save;
    const species = click(input, (value) => /Corydoras aeneus/.test(value), '选择搜索到的目标物种', '出现数量/日期并允许保存');
    if (species) return species;
    if (!/Record Existing Livestock/.test(visible)) {
      return click(input, (value) => /Record Existing Livestock|记录已有生物/.test(value), '从鱼缸主页选择“记录已有生物”入口', '打开已有生物记录流程')
        ?? { intentSummary: '主页面没有找到与记录已有生物目标匹配的入口', action: 'abandon', targetElementId: null, value: null, expectedResult: '停止并保留证据', confidence: 0.55 };
    }
  }

  if (goal.includes('每日检查')) {
    if (/Act now/.test(visible) && /增加打氧或水面扰动|立刻增加打氧或水面扰动/.test(visible)) {
      return { intentSummary: '风险等级和立即动作已经在结果页可见，检查目标完成', action: 'finish', targetElementId: null, value: null, expectedResult: '当前可见结果回答了自己的检查目标', confidence: 0.98 };
    }
    ...
    if (/Daily Aquarium Check/.test(visible)) {
      ...
      if (answered < dailyAnswers.length) {
        const answer = dailyAnswers[answered]!;
        const choice = click(input, (value) => value === answer, `按自己的观察回答第 ${answered + 1} 项：${answer}`, `检查进度进入 ${answered + 1} / 6`);
        if (choice) return choice;
      }
      const generate = click(input, (value) => /Generate Results|生成检查结果/.test(value), '六项观察已经回答，生成风险结果', '页面显示风险等级和下一步动作');
      if (generate) return generate;
    }
    return click(input, (value) => /Start today.?s check|开始.*检查|每日检查/.test(value), '从鱼缸主页选择“今天的检查”入口', '打开每日鱼缸检查')
      ?? { intentSummary: '主页面没有找到与每日检查目标匹配的入口', action: 'abandon', targetElementId: null, value: null, expectedResult: '停止并保留证据', confidence: 0.55 };
  }

  return { intentSummary: '当前页面没有与目标足够相关且安全的下一步', action: 'abandon', targetElementId: null, value: null, expectedResult: '停止并保留证据', confidence: 0.5 };
}

const actorRequests: Array<{ caseId: string; userPrompt: string }> = [];
const judgeRequests: Array<{ caseId: string; userPrompt: string }> = [];
...
function responder(request: AiStructuredRequest) {
  ...
  if (request.schemaName === 'agent_decision') {
    actorRequests.push({ caseId: String(request.metadata.caseId ?? 'unknown'), userPrompt: request.userPrompt });
  ...
  if (request.schemaName === 'semantic_judge_result') {
    judgeRequests.push({ caseId: String(request.metadata.caseId ?? 'unknown'), userPrompt: request.userPrompt });
...
const provider = new MockAiProvider(responder, 0, 'aquaguide-blind-scripted-v1');
...
const actorOracleLeaks = actorRequests.filter((request) => request.userPrompt.includes(oracleOnlyMarker));
const judgeOracleVisible = judgeRequests.length > 0 && judgeRequests.every((request) => request.userPrompt.includes(oracleOnlyMarker));
...
const diagnostic = {
  ...
  claimBoundary: 'Deterministic scripted Blind Actor validates Actor/Judge knowledge separation and real-browser plumbing; it is not a real-LLM usability study.',
  actorRequestCount: actorRequests.length,
  judgeRequestCount: judgeRequests.length,
  actorOracleLeakCount: actorOracleLeaks.length,
  judgeOracleVisible,
  allPassed,
  allBlind,
  falsePositiveFree,
...
await writeFile(resolve(outputDir, 'actor-requests.json'), JSON.stringify(actorRequests.map((item) => ({ caseId: item.caseId, oracleMarkerPresent: item.userPrompt.includes(oracleOnlyMarker) })), null, 2));

process.stdout.write(`AquaGuide Blind Experience: ${taskResults.filter((item) => item.verdict === 'pass').length}/${taskResults.length} Judge PASS\n`);
for (const item of taskResults) {
  process.stdout.write(`- ${item.caseId}: ${item.verdict}; actions=${item.actionCount}; frictions=${item.frictionCount}; findings=${item.findingCount}\n`);
}
process.stdout.write(`- Actor Oracle leaks: ${actorOracleLeaks.length}\n`);
...
if (actorOracleLeaks.length > 0) throw new Error('Hidden Oracle marker leaked into an Actor request.');
if (!judgeOracleVisible) throw new Error('Independent Judge did not receive the hidden Oracle marker for every task.');
if (!falsePositiveFree) throw new Error('Scripted ideal blind path produced a UX false positive; inspect the evidence before accepting it.');
