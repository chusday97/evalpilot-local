...
function findButton(input: any, predicate: (value: string) => boolean) {
  return (input.observation?.interactableElements ?? []).find((item: any) =>
    !item.disabled && item.tagName === 'button' && predicate(label(item)));
}
...
function click(
  ...
  const target = findButton(input, predicate);
  return target
...
function actorDecision(input: any) {
  ...
  if (goal.includes('60×30×30') && goal.includes('淡水')) {
    ...
    if (pageUrl.includes('/welcome')) {
      return click(input, (value) => /Build your first aquarium|建立第一个鱼缸/.test(value), '选择与建立第一个鱼缸最直接相关的入口', '进入鱼缸创建流程')
    ...
    if (!settingsOpen) {
      return click(input, (value) => /Create or configure a tank|Build or complete aquarium|建立或完善鱼缸|Tank Settings|鱼缸设置/.test(value), '选择建缸/鱼缸设置入口', '出现尺寸和水体设置')
    ...
    const dimensionsIncomplete = Boolean(dimensions && /Incomplete dimensions|尺寸未记录/.test(label(dimensions)));

    // Only treat numeric fields as dimensions when the three dimension inputs are visible at
    // the same time. A single numeric field inside Parameters is the temperature target and is
    // deliberately left unknown because the persona never supplied it.
    if (numberFields.length >= 3) {
      ...
      if (empty) {
        ...
        return {
          intentSummary: `填写自己已知的第 ${index + 1} 个尺寸`,
          action: 'fill',
          targetElementId: empty.elementId,
          value,
          expectedResult: '该尺寸字段保留输入值',
          confidence: 0.95,
        };
      }
    }
    if (dimensionsIncomplete) {
      return {
        intentSummary: '尺寸尚未记录，打开尺寸设置',
        action: 'click',
        targetElementId: dimensions.elementId,
        value: null,
        expectedResult: '显示长宽高三个输入框',
        confidence: 0.9,
      };
    }

    const parameters = findButton(input, (value) => /Parameters|参数/.test(value));
    const waterTypeUnknown = Boolean(parameters && /Water type unknown|水体未记录/.test(label(parameters)));
    if (waterTypeUnknown) {
      const freshwaterOption = findButton(input, (value) => /^淡水(?:常见观赏鱼)?$/.test(value) || /^Freshwater/.test(value));
      if (freshwaterOption) {
        return {
          intentSummary: '选择自己已知的淡水类型',
          action: 'click',
          targetElementId: freshwaterOption.elementId,
          value: null,
          expectedResult: '水体类型显示为 Freshwater/淡水',
          confidence: 0.95,
        };
      }
      return parameters
        ? { intentSummary: '水体类型尚未记录，打开 Parameters', action: 'click', targetElementId: parameters.elementId, value: null, expectedResult: '出现淡水/海水选项', confidence: 0.9 }
        : abandon('鱼缸设置里找不到水体类型入口');
    }

    return click(input, (value) => /^Save Settings$|^保存设置$/.test(value), '目标中已知的尺寸和淡水类型都已填写，保存设置；未知温度保持空白', '返回鱼缸主页面并保留配置')
      ?? abandon('已知设置完成，但找不到保存入口');
  }
  ...
  if (goal.includes('Corydoras aeneus')) {
    ...
    if (!search) {
      return click(input, (value) => /Record Existing Livestock|记录已有生物/.test(value), '从鱼缸主页选择记录已有生物入口', '打开已有生物记录流程')
        ?? abandon('鱼缸主页面没有找到与记录已有生物目标匹配的入口');
    }

    if (!search.currentValuePresent) {
      return {
        intentSummary: '用自己知道的学名搜索生物',
        action: 'fill',
        targetElementId: search.elementId,
        value: 'Corydoras aeneus',
        expectedResult: '出现目标物种候选',
        confidence: 0.95,
      };
    }

    const save = click(input, (value) => /^保存到鱼缸$/.test(value) || /^Save to.*tank$/i.test(value), '把选中的已有生物保存到当前鱼缸', '页面确认记录已保存');
    if (save) return save;
    const species = click(input, (value) => /^Corydoras aeneusCorydoras aeneus/.test(value), '选择与自己目标学名一致的候选', '出现数量和保存操作');
    return species ?? abandon('搜索已完成，但没有找到目标物种候选或保存入口');
  }
  ...
  if (goal.includes('每日检查')) {
    ...
    if (/Daily Aquarium Check/.test(visible)) {
      ...
      if (answered < answers.length) {
        ...
        const choice = click(input, (value) => value === answer, `按自己的观察回答第 ${answered + 1} 项：${answer}`, `检查进度进入 ${answered + 1} / 6`, 0.95);
        if (choice) return choice;
      }
      return click(input, (value) => /^Generate Results$|^生成检查结果$/.test(value), '六项观察已经回答，生成风险结果', '显示风险等级和下一步动作')
        ?? abandon('每日检查已打开，但找不到当前问题选项或生成结果入口');
    }

    return click(input, (value) => /Start today.?s check|开始.*检查/.test(value), '从鱼缸主页选择今天的检查入口', '打开每日鱼缸检查')
...
function responder(request: AiStructuredRequest) {
  ...
  if (request.schemaName === 'agent_decision') {
    actorRequests.push({
      ...
      markerPresent: request.userPrompt.includes(oracleOnlyMarker),
    });
  ...
  if (request.schemaName === 'semantic_judge_result') {
    ...
    if (deterministic.hardFailure) {
      return {
        ...
        whatWorked: [],
        whatFailed: ['成功条件未成立'],
        whyItMatters: ['用户目标没有完成'],
        confirmedFacts: ['确定性断言失败'],
        hypotheses: [],
        unknowns: [],
    ...
    return passed
      ? {
        ...
        summary: '独立 Judge 确认 Blind Actor 已完成任务。',
        whatWorked: ['全部确定性成功信号成立'],
        whatFailed: [],
        whyItMatters: [],
        confirmedFacts: ['真实任务完成信号可见'],
        hypotheses: [],
        unknowns: [],
        evidenceRefs: deterministic.evidenceRefs ?? [],
        confidence: 0.95,
...
const provider = new MockAiProvider(responder, 0, 'aquaguide-blind-scripted-v3');
...
const diagnostic = {
  ...
  claimBoundary: 'Deterministic scripted Blind Actor validates Actor/Judge knowledge separation, neutral-entry discovery, known-information fidelity and real-browser plumbing; it is not a real-LLM usability study.',
...
process.stdout.write(`AquaGuide Blind Experience v3: ${taskResults.filter((item) => item.verdict === 'pass').length}/${taskResults.length} Judge PASS\n`);
for (const item of taskResults) {
  process.stdout.write(`- ${item.caseId}: ${item.verdict}; actions=${item.actionCount}; backtracks=${item.backtrackCount}; frictions=${item.frictionCount}; findings=${item.findingCount}; fills=${JSON.stringify(item.fillValues)}\n`);
}
...
process.stdout.write(`- Known-information faithful: ${knownInformationFaithful}\n`);
...
if (!knownInformationFaithful) throw new Error('Blind Actor filled information that was not supplied by the persona/case.');
