...
const setupEquivalenceNames: Record<string, string> = {
  unique: '唯一 baseline',
  exact_signature_match: '等价重复 baseline',
  missing: '缺少 baseline',
  ambiguous: '状态冲突',
};

function SetupKnowledgeCards({ items }: { items: Json[] }) {
  if (!items.length) return null;
  return <><h3>Setup 状态来源</h3><div className="adaptive-detail-grid">{items.map((source, index) => {
    const candidates = source.candidateCaseIds ?? [];
    const fingerprint = source.setupStateFingerprint ? String(source.setupStateFingerprint) : '';
    return <article key={`${source.setupTaskId ?? 'setup'}-${index}`}><span>{source.setupTaskId ?? 'Setup'}</span><b>{setupEquivalenceNames[source.equivalence] ?? source.equivalence ?? source.status}</b><small>{source.sourceCaseId ? `执行 ${source.sourceCaseId}` : `${candidates.length} 个候选未选定`}{fingerprint ? ` · signature ${fingerprint.slice(0, 12)}…` : ''}</small></article>;
  })}</div></>;
}
